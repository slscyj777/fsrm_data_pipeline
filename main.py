...
def run_pipeline(steps: list[str] | None = None, day: int | None = None, month: int | None = None, year: int | None = None) -> bool:
    ...
    sub_folder = SP_ROOT / settings.SUB_FOLDER_NAME /  month_folder_name(target_month, target_year)
    ...
    if "all" in requested_steps or "transform" in requested_steps:
        ...
        df_beer = extract_sfc_data(
            forecast_path, 
            sheet_name="BEER",
            columns_to_read=settings.BEER_COLUMNS_TO_READ,
            rename_map=settings.SFC_RENAME_MAPPING 
        )

        df_spirits = extract_sfc_data(
            forecast_path,
            sheet_name="Spirits", 
            columns_to_read=settings.SPIRITS_COLUMNS_TO_READ,
            rename_map=settings.SFC_RENAME_MAPPING
        )

        df_sku = extract_sku_data(
            sku_path,
            columns_to_read=settings.SKU_COLUMNS_TO_READ,
            rename_map=settings.SKU_RENAME_MAPPING,
        ...
        df = (extract_sermsuk_data(
            ...
            .pipe(rename_normalize_stock_columns,column_mapping = settings.ASSIGN_COLUMN_MAPPING
                                                ,stock_columns = settings.STOCK_COL
                                                ,ship_columns = settings.SHIP_COL
                                                )
            .pipe(map_base_unit_to_sku, sku_df = df_sku)
            .pipe(validate_extracted_data)
            .pipe(total_stock_as_crates, stock_columns = settings.STOCK_COL)
            .pipe(total_shippment_as_crates, ship_columns = settings.SHIP_COL)
            .pipe(map_sermsuk_to_TBL_WH_select_columns
                , mapping_df = df_mapping
                , column_to_map = "branch_code")
            .pipe(rearrange_columns, column_order = settings.ASSIGN_COLUMN_ORDER)
